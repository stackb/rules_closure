...
/**
 * Globally opts out of native promises.  Applies to the currently active test
 * case (if there is one), as well as all future test cases.  NOTE: this is a
 * last-ditch fallback.  Whenever possible, fix the test to work with native
 * promises, or else set `useGoogPromise` on the individual `TestCase` instead.
 */
goog.testing.TestCase.useGoogPromises = function() {
  goog.testing.TestCase.getActiveTestCase()?.useGoogPromise();
 ...
 * @private
 */
goog.testing.TestCase.prototype.useNativePromise_ = true;
...
goog.testing.TestCase.prototype.getActuallyRunCount = function() {
  ...
  let rv = 0;
  for (const key in this.testsToRun_) {
    rv++;
  }
  return rv;
};
...
goog.testing.TestCase.prototype.getTestResults = function() {
  'use strict';
  var map = {};
  var resultsByName = this.result_.resultsByName;
  for (const key in resultsByName) {
    // Make sure we only use properties on the actual map
    if (!Object.prototype.hasOwnProperty.call(resultsByName, key)) {
      continue;
    }
    var resultArray = resultsByName[key];
    map[key] = [];
...
goog.testing.TestCase.prototype.reportUnpropagatedAssertionExceptions_ =
  ...
  // Remove actualError from extraExceptions. If it isn't a JSUnit exception, it
  // won't be in this array.
  const index = extraExceptions.indexOf(
      /** @type {!goog.testing.JsUnitException} */ (actualError));
  if ((index >= 0)) {
    extraExceptions.splice(index, 1);
  }
...
goog.testing.TestCase.prototype.addTestObj_ = function(obj, name, objChain) {
  ...
  var properties = goog.testing.TestCase.getAllPropertyNames_(obj);
 ...
 * @public
 */
goog.testing.TestCase.getAllPropertyNames_ =
    function(obj) {
  if (!obj) {
    return [];
  }
  ...
  if (!Object.getOwnPropertyNames || !Object.getPrototypeOf) {
    return goog.testing.TestCase.getKeys_(obj);
  }
  ...
  return goog.testing.TestCase.getKeys_(visitedSet);
}
 ...
 */
goog.testing.TestCase.getKeys_ = function(obj) {
  const res = [];
  let i = 0;
...
goog.testing.TestCase.prototype.recordError = function(testName, error) {
  'use strict';
  if (error instanceof
      goog.module.get('goog.testing.pending').TestMarkedAsPending) {
    this.result_.suppressedTests.push(testName);
    return;
  }
...
goog.testing.TestCase.prototype.logError = function(name, error) {
  ...
  var stack = this.cleanStackTrace_(error.stack, error.message);
  var err = new goog.testing.TestCase.Error(name, error.message, stack);
...
goog.testing.TestCase.Result = function(testCase) {
  ...
  /**
   * Which tests were suppressed by calling `pending()` or by `shouldRunTests()`
   * returning false.
   * @type {!Array<string>}
   */
...
goog.testing.TestCase.parseRunTests_ = function(href) {
  ...
  // doesn't support
  const searchParams =
      new URL('https://google.com' + nonOriginParts).searchParams;
